...
void score(int i)
   {
    ...
    draw_printf(320,242,"GAGNE! Score: %d/1000",i);
    update_graphics();
    sleep(5);
    get_key();
    clear_screen();
...
{   set_font(font_HELVETICA_18);
    set_drawing_color(color_BLACK);
    draw_printf(x,y,"L'ordinateur va essayer de sortir du labyrinthe");
    draw_printf(x,y-t,"Sera-'-il meilleur que vous?");
    draw_printf(x,y-2*t,"Appuyez sur une touche pour commencer");
    update_graphics();
    get_key();
    clear_screen();
}
...
y2=co+(t/2);
touche=dir();
draw_circle_full(finco,finli,r);
while((x2>finli)&&(y2<finco))
{
